...
class AccessibilityActions:
    def accessibility_describe_screen():
        ...
        if not os.path.exists(python_executable):
            app.notify("ERROR: Python executable not found", f"Path does not exist: {python_executable}")
            return

        if not os.path.exists(script_path):
            app.notify("ERROR: Agent script not found", f"Path does not exist: {script_path}")
            return
            
        try:
            proc = subprocess.Popen([python_executable, script_path], cwd=script_dir, stdout=subprocess.PIPE, stderr=subprocess.PIPE, creationflags=subprocess.CREATE_NO_WINDOW)
            stdout, stderr = proc.communicate()
            if stdout:
                print(f"Agent STDOUT: {stdout.decode(errors='ignore')}")
            if stderr:
                print(f"Agent STDERR: {stderr.decode(errors='ignore')}", file=sys.stderr)
    ...
    def accessibility_test():
        ...
        def accessibility_test_output():
            ...
            if not os.path.exists(script_path):
                ...
                app.notify("ERROR: NVDA test script not found", f"Path does not exist: {script_path}")
            ...
            try:
                ...
                if stderr:
                    ...
                    print(f"NVDA Test STDERR: {stderr.decode(errors='ignore')}", file=sys.stderr)

            except Exception as e:

                app.notify("ERROR: Failed to run NVDA test script", f"Subprocess error: {e}")
